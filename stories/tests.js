...
storiesOf("React Twitch Embed Video", module)
  ...
  .add("Layout: with or without chat", () => (
    <TwitchEmbedVideo
      channel="talk2megooseman"
      layout={select("Layout", ["video-with-chat", "video"], "video")}
    />
  ))
  .add("Choose autoplay", () => (
    <TwitchEmbedVideo
      video="462014255"
      autoplay={select("Autoplay", [true, false], false)}
    />
  ))
  .add("Choose auto mute", () => (
    <TwitchEmbedVideo
      video="462014255"
      muted={select("Muted", [true, false], true)}
    />
  ))
  .add("2 twitch streams and dynamic update knob", () => (
    <div style={{ height: "900px" }}>
      ...
      <TwitchEmbedVideo
        width="100%"
        height="100%"
        channel={text("Second Embed Channel", "lana_lux")}
        targetId="second"
      />
    </div>
  ))
